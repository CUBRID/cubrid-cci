...
static int read_catalog_file (unsigned int *file_id, unsigned int *sql_id, char *path, bool need_to_create);
 ...
 */
static int
read_catalog_file (unsigned int *file_id, unsigned int *sql_id, char *path, bool need_to_create)
{
  FILE *fp = NULL;
  char buf[LINE_MAX];
...
apply_sql_logs (int conn)
{
  ...
  while (ca_Info.curr_file_id <= ca_Info.src_file_id)
    {
      if (snprintf (sql_log_path, PATH_MAX - 1, "%s.%u", base_log_path, ca_Info.curr_file_id) < 0)
	{
	  assert (false);
	  return ER_CA_FAILED;
	}
